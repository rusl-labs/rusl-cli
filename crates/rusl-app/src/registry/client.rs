...
impl RegistryClient {
    pub fn new(config: Config) -> Self {
        Self::with_api(
            RuslApiClient::new(config.api_base_url).with_acting_account(config.acting_account),
        )
    }
    ...
    pub fn with_user_agent(config: Config, user_agent: impl Into<String>) -> Self {
        Self::with_api(
            RuslApiClient::new(config.api_base_url)
                ...
                .with_acting_account(config.acting_account),
        )
    }
    ...
    ) -> Result<models::RuslWebApiSchemaControllerCreate201Response> {
        let (mut credentials, mut session) = self.load_session()?;
        let response = self
            .api
