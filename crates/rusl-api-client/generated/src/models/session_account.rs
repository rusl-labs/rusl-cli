/*
 * Rusl CLI API
 *
 * API for the Rusl CLI
 *
 * The version of the OpenAPI document: 0.1.0
 *
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// SessionAccount : Account information for user session
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct SessionAccount {
    /// Type discriminator
    #[serde(rename = "__typename")]
    pub __typename: Typename,
    /// Avatar asset ID
    #[serde(rename = "avatar_asset_id", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub avatar_asset_id: Option<Option<uuid::Uuid>>,
    /// Display name
    #[serde(rename = "display_name", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub display_name: Option<Option<String>>,
    /// Account global ID
    #[serde(rename = "guid")]
    pub guid: String,
    /// Owner user ID
    #[serde(rename = "owner_user_id")]
    pub owner_user_id: uuid::Uuid,
    #[serde(rename = "permissions")]
    pub permissions: Box<models::SessionAccountPermissions>,
    /// User's roles in this account
    #[serde(rename = "roles")]
    pub roles: Vec<Roles>,
    /// Account slug
    #[serde(rename = "slug")]
    pub slug: String,
    /// Account type
    #[serde(rename = "type")]
    pub r#type: Type,
}

impl SessionAccount {
    /// Account information for user session
    pub fn new(__typename: Typename, guid: String, owner_user_id: uuid::Uuid, permissions: models::SessionAccountPermissions, roles: Vec<Roles>, slug: String, r#type: Type) -> SessionAccount {
        SessionAccount {
            __typename,
            avatar_asset_id: None,
            display_name: None,
            guid,
            owner_user_id,
            permissions: Box::new(permissions),
            roles,
            slug,
            r#type,
        }
    }
}
/// Type discriminator
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum Typename {
    #[serde(rename = "accounts")]
    Accounts,
}

impl Default for Typename {
    fn default() -> Typename {
        Self::Accounts
    }
}
/// User's roles in this account
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum Roles {
    #[serde(rename = "OWNER")]
    Owner,
    #[serde(rename = "CONTRIBUTOR")]
    Contributor,
}

impl Default for Roles {
    fn default() -> Roles {
        Self::Owner
    }
}
/// Account type
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum Type {
    #[serde(rename = "user")]
    User,
    #[serde(rename = "organization")]
    Organization,
}

impl Default for Type {
    fn default() -> Type {
        Self::User
    }
}
