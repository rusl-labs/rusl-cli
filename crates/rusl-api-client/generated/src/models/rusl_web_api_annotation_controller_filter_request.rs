...
/// RuslWebApiAnnotationControllerFilterRequest : Choose one pagination family per request: `first`/`after` for forward cursor pagination, `last`/`before` for backward cursor pagination, `page`/`page_size` for page-based pagination, or `limit`/`offset` for offset pagination. If you omit pagination params, this schema defaults to `offset` pagination with a default size of 20.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct RuslWebApiAnnotationControllerFilterRequest {
    ...
    pub filters:
        Option<Option<Vec<models::RuslWebApiAnnotationControllerFilterRequestFiltersInner>>>,
...
impl RuslWebApiAnnotationControllerFilterRequest {
    /// Choose one pagination family per request: `first`/`after` for forward cursor pagination, `last`/`before` for backward cursor pagination, `page`/`page_size` for page-based pagination, or `limit`/`offset` for offset pagination. If you omit pagination params, this schema defaults to `offset` pagination with a default size of 20.
    pub fn new() -> RuslWebApiAnnotationControllerFilterRequest {
